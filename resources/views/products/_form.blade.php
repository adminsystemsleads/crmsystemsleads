...
<div>
  <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Imagen (opcional)') }}</label>
  @isset($edit)
  ...
  <p class="text-[10px] text-gray-400 mt-1">{{ __('JPG, PNG o WebP. Máx 2MB.') }}</p>
  @isset($edit)
...
<div>
  <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Descripción') }}</label>
...
<div>
  <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Precio *') }}</label>
...
<div class="flex items-center gap-2">
  ...
  <label for="chkActive-{{ $sfx }}" class="text-sm text-gray-700">{{ __('Producto activo') }}</label>
</div>
