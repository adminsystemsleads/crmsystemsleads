    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="max-w-3xl mx-auto p-6">
          ...
          <h1 class="text-2xl font-semibold mb-6">{{ __('Categorías de Pago') }}</h1>
          ...
          @if (Auth::user()->hasTeamRole(Auth::user()->currentTeam, 'admin'))
            <form action="{{ route('categorias.store') }}" method="POST"
              ...
              <input type="text" name="nombre"
                     placeholder="{{ __('Nueva categoría (ej. Agua, Luz, Mantenimiento)') }}"
              ...
              <div class="flex items-center justify-between p-4">
                <div class="min-w-0">
                  ...
                  <div class="text-xs text-gray-500 mt-1">
                    {{ __('Usos en gastos:') }} <span class="font-semibold">{{ $categoria->gastos_count }}</span>
                  </div>
                </div>

                @if (Auth::user()->hasTeamRole(Auth::user()->currentTeam, 'admin'))
                  @php
                    $confirmMsg = '⚠️ ' . __('Esta acción eliminará la categoría «:nombre».', ['nombre' => $categoria->nombre]);
                    if ($categoria->gastos_count > 0) {
                      $confirmMsg .= ' ' . __('Actualmente está vinculada a :count gasto(s). Deberás reasignarlos o eliminarlos antes.', ['count' => $categoria->gastos_count]);
                    }
                    $confirmMsg .= "\n\n" . __('¿Deseas continuar?');
                  @endphp
                  <form action="{{ route('categorias.destroy', $categoria) }}" method="POST"
                    ...
                    @csrf
                    @method('DELETE')
                    <button class="text-red-600 hover:text-red-700 text-sm">{{ __('Eliminar') }}</button>
                  </form>
                @endif
              </div>
            @empty
              <div class="p-6 text-gray-500">{{ __('Aún no hay categorías.') }}</div>
            @endforelse
          </div>
