            ...
            <div class="bg-white shadow sm:rounded-lg p-6">
                <form action="{{ route('team.perfiles.updateMember', $member) }}" method="POST" enctype="multipart/form-data" class="space-y-5"
                    ...
                    <div class="flex flex-col items-center pb-6 mb-2 border-b border-gray-100">
                        ...
                        <p class="text-[11px] text-gray-400 mt-2">{{ __('JPG, PNG, GIF o WEBP · máx. 2 MB') }}</p>
                    ...
                    {{-- Notas adicionales --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Notas adicionales') }}</label>
                        <textarea name="notas" rows="4" maxlength="2000"
                                  class="w-full rounded-md border-gray-300 focus:ring-indigo-500 focus:border-indigo-500"
                                  placeholder="{{ __('Información relevante') }}">{{ old('notas', $profile->notas) }}</textarea>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            ...
                            @if ($onlyOneUser || $isEditingSelf)
                                <span class="text-xs font-normal text-gray-400 ml-1">{{ __('(no editable)') }}</span>
                            @endif
                        </label>
                        ...
                        @if ($onlyOneUser)
                            ...
                            <div class="w-full rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">
                                <strong>{{ $profile->crmRole?->name ?? __('Administrador (por defecto)') }}</strong>
                            </div>
                            <p class="text-xs mt-1"
                               style="color:#A08544;">
                                ⚠️ {{ __('El equipo solo tiene 1 usuario. No se puede modificar su rol porque dejaría al sistema sin administrador. Invita a otro usuario al equipo para poder gestionar roles.') }}
                            </p>
                        @elseif ($isEditingSelf)
                            ...
                            <p class="text-xs mt-1"
                               style="color:#A08544;">
                                ⚠️ {{ __('No puedes cambiar tu propio rol. Pídele a otro administrador del equipo que lo modifique.') }}
                            </p>
                        @else
                            <select name="crm_role_id"
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}"
                                        ...
                                        {{ $role->name }}@if ($role->is_default) {{ __('(sistema)') }} @endif
                                    </option>
                                @endforeach
                            </select>
                            <p class="text-xs text-gray-500 mt-1">
                                {{ __('Para crear o modificar roles ve a') }}
                                <a href="{{ route('team.crm-roles.index') }}" style="color:#1E2E48;" class="underline">{{ __('Configurar Roles y Permisos de CRM') }}</a>.
                            </p>
                        @endif
                    </div>
