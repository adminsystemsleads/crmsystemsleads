...
<div class="max-w-3xl mx-auto px-4 py-8">
  ...
  <div class="flex items-center gap-3 mb-6">
    ...
    <h1 class="text-xl font-bold text-gray-900">{{ __('Configuración de Facturación Electrónica') }}</h1>
  </div>
  ...
  <form method="POST" action="{{ route('invoice-config.update') }}" class="space-y-6">
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1">
          <label for="test_mode" class="text-sm font-bold text-amber-800 cursor-pointer">
            {{ __('Modo prueba (simulado)') }}
          </label>
          <p class="text-xs text-amber-700 mt-0.5">
            {{ __('Activa esto para probar la facturación') }} <strong>{{ __('sin enviar nada a SUNAT') }}</strong>.
            {{ __('Los comprobantes se marcan como "Aceptado" automáticamente. No necesitas certificado ni credenciales reales.') }}
          </p>
          <button type="button" onclick="fillTestData()"
            ...
            {{ __('Rellenar con datos de prueba') }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
      <h2 class="text-sm font-bold text-gray-800 border-b pb-2">{{ __('Datos del Emisor (SUNAT)') }}</h2>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('RUC *') }}</label>
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Razón Social *') }}</label>
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Nombre Comercial') }}</label>
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('País') }}</label>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Ubigeo *') }}</label>
          <input type="text" name="ubigeo" required maxlength="6"
                 value="{{ old('ubigeo', $config->ubigeo ?? '150101') }}"
                 class="w-full rounded-lg border-gray-200 text-sm py-2">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Departamento *') }}</label>
          <input type="text" name="departamento" required maxlength="100"
                 value="{{ old('departamento', $config->departamento ?? 'LIMA') }}"
                 class="w-full rounded-lg border-gray-200 text-sm py-2">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Provincia *') }}</label>
          <input type="text" name="provincia" required maxlength="100"
                 value="{{ old('provincia', $config->provincia ?? 'LIMA') }}"
                 class="w-full rounded-lg border-gray-200 text-sm py-2">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Distrito *') }}</label>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Serie Factura') }}</label>
          <input type="text" name="serie_factura" maxlength="4"
                 value="{{ old('serie_factura', $config->serie_factura ?? 'F001') }}"
                 class="w-full rounded-lg border-gray-200 text-sm py-2">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Serie Boleta') }}</label>
    ...
    {{-- Credenciales SUNAT --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
      <h2 class="text-sm font-bold text-gray-800 border-b pb-2">{{ __('Credenciales SOL / Ambiente') }}</h2>

      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Usuario SOL') }}</label>
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Clave SOL') }}</label>
          <input type="password" name="sol_password" maxlength="50"
                 value="{{ old('sol_password', $config->sol_password ?? '') }}"
                 class="w-full rounded-lg border-gray-200 text-sm py-2">
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Ambiente') }}</label>
          <select name="ambiente" class="w-full rounded-lg border-gray-200 text-sm py-2">
            <option value="beta"       {{ old('ambiente', $config->ambiente ?? 'beta') === 'beta'       ? 'selected' : '' }}>{{ __('Beta (pruebas)') }}</option>
            <option value="produccion" {{ old('ambiente', $config->ambiente ?? 'beta') === 'produccion' ? 'selected' : '' }}>{{ __('Producción') }}</option>
          </select>
        </div>
      </div>

      <div>
        <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('Certificado PEM') }}</label>
        ...
        <p class="text-[11px] text-gray-400 mt-1">{{ __('Pega aquí el contenido del archivo .pem de tu certificado digital.') }}</p>
      </div>
    </div>
