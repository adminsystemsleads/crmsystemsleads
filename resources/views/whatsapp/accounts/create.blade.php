      ...
      <div class="bg-white shadow-sm sm:rounded-lg p-6">
        ...
        <form method="POST" action="{{ route('whatsapp.accounts.store') }}">
          ...
          <div class="mb-4">
            ...
            <input name="phone_number_id" value="{{ old('phone_number_id') }}" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
            ...
            @else
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto pr-1">
                @foreach($teamMembers as $member)
                  @php $checked = in_array($member->id, old('assignee_ids', [])); @endphp
          ...
          <p class="mt-4 text-xs text-gray-500">
            Webhook URL: <span class="font-mono">{{ url('/webhooks/whatsapp') }}</span>
          </p>
        </form>
      </div>
    </div>
