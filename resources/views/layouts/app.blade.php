...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  <head>
    ...
    <style>
      ...
      .notif-bell { position: fixed !important; top: 1rem; right: 1rem; z-index: 40; transition: opacity .25s ease, transform .25s ease; }
      .notif-bell.notif-hidden { opacity: 0; transform: translateY(-200%); pointer-events: none; }
      ...
      .notif-panel { position: absolute; top: calc(100% + .5rem); right: 0; width: 40rem; max-width: calc(100vw - 1.5rem);
        max-height: 85vh; overflow-y: auto; background: #fff; color: #374151; border: 1px solid #e5e7eb;
        border-radius: .75rem; box-shadow: 0 12px 28px rgba(15,23,42,.18); }
      .notif-empty { min-height: 14rem; display: flex; align-items: center; justify-content: center; }
